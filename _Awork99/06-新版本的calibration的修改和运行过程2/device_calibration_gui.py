...
from utils.gains_polling_helper import poll_for_results
from utils.siu_device_selector import SiuDeviceSelector
...
from utils.helpers import get_files_in_folder, alert_window, check_and_install_update, ScrollMessageArea, clear_layout, \
    alert_dialog, load_saved_value_to_combo_box
...
class DeviceCalibrationApp(DeviceCalibrationBase, QMainWindow):
    ...
    SIU_TYPE_SETTING_KEY = 'dc_siu_type_setting'
    DEVICE_TYPE_SETTING_KEY = 'dc_device_type_setting'
    ...
    def __init__(self, app_root_name):
        ...
        # allow combo boxes to take up space
        setup_box_layout.setColumnStretch(1, 2)
        setup_box_y = 0
        # Add SIU and Device Part selector
        hw_list = [hw.description for hw in ommo.Hardware if hw.is_siu]
        device_list = [device.description for device in ommo.DevicePart]
        self.siu_device_selector = SiuDeviceSelector(hw_list, device_list, 102)
        setup_box_layout.addWidget(self.siu_device_selector, setup_box_y, 0, 2, 2)
        # Add buttons
        ...
        self.siu_device_selector.siu_changed.connect(lambda selected_text: QSettings()
                                                     .setValue(self.SIU_TYPE_SETTING_KEY, selected_text))
        self.siu_device_selector.device_changed.connect(lambda selected_text: QSettings()
                                                        .setValue(self.DEVICE_TYPE_SETTING_KEY, selected_text))
    ...
    def device_part_changed(self, selected_text):
        selected_device_part = ommo.DevicePart.from_desc(selected_text)
        if selected_device_part.device_proto.onboard_siu_parent != pb.SIU_PID_UNKNOWN:
            self.siu_device_selector.set_sius_combo_enabled(False)
            siu = ommo.Hardware.from_pid(selected_device_part.device_proto.onboard_siu_parent)
            self.siu_device_selector.set_siu(siu.description)
        else:
            self.siu_device_selector.set_sius_combo_enabled(True)
    ...
    def load_settings(self):
        ...
        prev_selected_siu = settings.value(self.SIU_TYPE_SETTING_KEY, '')
        if prev_selected_siu:
            self.siu_device_selector.set_siu(prev_selected_siu)

        prev_selected_device_part = settings.value(self.DEVICE_TYPE_SETTING_KEY, '')
        if prev_selected_device_part:
            self.siu_device_selector.set_device(prev_selected_device_part)
    ...
    def setup(self):
        ...
        selected_siu = ommo.Hardware.from_desc(self.siu_device_selector.get_siu())
        selected_device_part = ommo.DevicePart.from_desc(self.siu_device_selector.get_device())
        other_ports = []
        ...
        if self.matched_sius:
            ...
            for siu in self.matched_sius:
                ...
                success = siu.set_onboard_sensor_enabled(selected_device_part.device_proto.onboard_siu_parent != pb.SIU_PID_UNKNOWN)
                if not success:
                ...
                for port in range(siu.get_num_ports()):
    ...
    def run_all(self):
        ...
        if self.is_setup and self.calibrate_devices_box_enabled_cb.isChecked() and self.upload_gains_box_enabled_cb.isChecked():
            ...
            gain_file_path = os.path.join(os.path.splitext(data_file_path)[0], 'v0.16.5/sensor_calibration.hdf5')
    ...
    def upload_result_callback(self, result_code: int, uploaded_filename: str):
        if result_code == 0:
            ...
            self.result_worker, self.worker_thread = poll_for_results(data_file_path=uploaded_filename,
    ...
    def run_sensor_calibration_locally(self, data_file_path):
        ...
        calib_command += ' ommotech/analysis:v0.16.5 python /opt/ommoalg/ommoalg/calibrate_sensor.py'
